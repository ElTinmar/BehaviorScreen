...
rt_index = bouts_category_name_short.index("AS")
hat_index = bouts_category_name_short.index("S1")
...
sub_df = df[(df['epoch_name'] == "pinwheel break after clockwise") | (df['epoch_name'] == "pinwheel break after counter-clockwise")].copy()
...
ax_ipsi.set_ylabel('Ipsi AS+S1 turn Frequency (Hz)', fontsize=12, fontweight='bold')
...
ax_contra.set_ylabel('Contra AS+S1 Frequency (Hz)', fontsize=12, fontweight='bold')
ax_contra.grid(True, linestyle=':', alpha=0.5)

plt.tight_layout()
plt.savefig(
    'temporal_dynamics_okr_MAE.svg', 
    bbox_inches='tight'
)
plt.savefig(
    'temporal_dynamics_okr_MAE.png',
    dpi=300, 
    bbox_inches='tight'
)
plt.show()
...
plt.tight_layout()
plt.savefig(
    'temporal_dynamics_okr_MAE_index.svg', 
    bbox_inches='tight'
)
plt.savefig(
    'temporal_dynamics_okr_MAE_index.png',
    dpi=300, 
    bbox_inches='tight'
)
plt.show()