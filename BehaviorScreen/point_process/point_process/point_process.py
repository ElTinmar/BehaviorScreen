...
def _fit_single_bootstrap(
    seed_seq, dataset: PointProcessDataset, model: "PointProcess"
):
...
class PointProcess:
    ...
    def fit(self, dataset: PointProcessDataset, method: str = "L-BFGS-B", **kwargs):
        ...
        valid = res.success and np.isfinite(res.fun) and np.all(np.isfinite(res.x))
        if not valid:
            raise RuntimeError(
    ...
    def _sampling_bounds(
        self, i: int, default: np.ndarray, fallback_scale: float = 10.0
    ):
        lo, hi = self.bounds[i]
        d = default[i]
        if lo is None:
    ...
    def _generate_multistart_inits(
        ...
        log_scale = (
            ...
            else self._infer_log_scale_params()
        )
        ...
        for _ in range(n_jitter):
            ...
            for i in range(k):
                ...
                if name in log_scale and default[i] > 0:
                    ...
                    span = (
                        (hi - lo) if np.isfinite(hi - lo) else max(abs(default[i]), 1.0)
                    )
    ...
    def fit_multistart(
        ...
        **kwargs,
        ...
        inits = self._generate_multistart_inits(
            rng, n_starts, log_scale_params=log_scale_params
        )
        ...
        if not valid:
            raise RuntimeError(
                f"All {n_starts} multistart optimizations failed for {self.name}."
            )
        ...
        status = (
            "OK (looks unimodal)"
            if n_at_global == n_converged
            else f"WARNING: {n_converged - n_at_global} start(s) converged to a DIFFERENT, worse optimum "
            f"-- {self.name} may be poorly identified; treat AIC comparisons involving it with caution."
        )
        print(
            f"[{self.name}] multistart: {n_converged}/{n_starts} converged, "
    ...
    def bootstrap(
        ...
        with tqdm_joblib(tqdm(total=n_boot, desc="Bootstrap")):
            boot_results = joblib.Parallel(n_jobs=n_jobs)(
                joblib.delayed(_fit_single_bootstrap)(s, dataset, self) for s in seeds
            )
    ...
    def simulate_dataset(
        ...
        for f_idx, t_idx, _ in template.iter_streams():
            ...
            event_trials.append(np.full(len(simulated_events), t_idx, dtype=int))
            event_fish.append(np.full(len(simulated_events), f_idx, dtype=int))
        ...
        return PointProcessDataset(
            ...
            event_fish_idx=(
                np.concatenate(event_fish) if event_fish else np.array([], dtype=int)
            ),
    ...
    def gof_statistics(
        ...
        max_abs_event_acf = (
            float(np.max(np.abs(finite_acf))) if len(finite_acf) > 0 else np.nan
        )
        ...
        return {
            ...
            "zero_stream_fraction": (
                float(np.mean(stream_counts == 0)) if len(stream_counts) else np.nan
            ),
            "multiple_event_stream_fraction": (
                float(np.mean(stream_counts >= 2)) if len(stream_counts) else np.nan
            ),
            "mean_stream_count": (
                float(np.mean(stream_counts)) if len(stream_counts) else np.nan
            ),
            "variance_stream_count": (
                float(np.var(stream_counts)) if len(stream_counts) else np.nan
            ),
            "mean_fish_total": (
                float(np.mean(fish_totals)) if len(fish_totals) else np.nan
            ),
            "variance_fish_total": (
                float(np.var(fish_totals)) if len(fish_totals) else np.nan
            ),
            "max_fish_total": (
                float(np.max(fish_totals)) if len(fish_totals) else np.nan
            ),
            # Residual metadata.
    ...
    def parametric_gof_bootstrap(
        ...
        if not 0.0 < min_envelope_fraction <= 1.0:
            raise ValueError("min_envelope_fraction must lie in (0, 1].")
        ...
        for name in scalar_names:
            ...
            bootstrap_percentile = 100.0 * np.mean(bootstrap_values <= observed_value)

            if name in upper_tail_discrepancies and np.isfinite(observed_value):
                p_upper = (1.0 + np.sum(bootstrap_values >= observed_value)) / (
            ...
            if np.isfinite(observed_value):
                lower_tail = (1.0 + np.sum(bootstrap_values <= observed_value)) / (
                    len(bootstrap_values) + 1.0
                )

                upper_tail = (1.0 + np.sum(bootstrap_values >= observed_value)) / (
                    len(bootstrap_values) + 1.0
                )
            ...
            summary_records.append(
                {
                    ...
                    "ci_lower": float(np.percentile(bootstrap_values, alpha)),
                    "ci_upper": float(
                        np.percentile(
                            bootstrap_values,
                            100.0 - alpha,
        ...
        common_grid = reliable_grid & np.isfinite(observed_curve)
        ...
        if len(common_indices) > 0:
            ...
            if common_last_idx >= 0:
                common_mask = np.arange(len(u_grid)) <= common_last_idx
                ...
                if n_simultaneous_curves > 0:
                    ...
                    simultaneous_lower[common_mask] = np.clip(
                        u_grid[common_mask] - simultaneous_critical_value,
                        0.0,
                        1.0,
                    )
        ...
        bootstrap_statistics = pd.DataFrame(bootstrap_scalar_records)
    ...
    def plot_parametric_gof_calibration(
        ...
        ks_row = summary.loc[summary["statistic"] == "calibration_ks"].iloc[0]
        ...
        for i in range(k):
            for j in range(i, k):
                if i == j:
                    ...
                    val = (obj(p_pp) - obj(p_pm) - obj(p_mp) + obj(p_mm)) / (
                        4 * h[i] * h[j]
                    )
                    hessian[i, j] = val
                    hessian[j, i] = val
    ...
    def _autocorrelation(
        z_seqs: List[np.ndarray], max_lags: int
    ...
    def _pool_stream_residuals(
        ...
        np.ndarray, np.ndarray, Dict[int, List[Tuple[float, bool]]], List[np.ndarray]
    ]:
        """
        ...
        fish_pairs: Dict[int, List[Tuple[float, bool]]] = {
            f: [] for f in range(dataset.num_fish)
        }
    ...
    def time_rescaling(
        ...
        if self.params_ is None:
            raise ValueError(
                "Model must be fitted before running time-rescaling analysis."
            )

        all_vals_arr, all_censored_arr, fish_pairs, pooled_z_seqs = (
            self._pool_stream_residuals(dataset)
        )
        ...
        return {
            ...
            "median_fish_dn": (
                float(np.median(fish_dn_stats)) if len(fish_dn_stats) > 0 else np.nan
            ),
            "mean_fish_dn": (
                float(np.mean(fish_dn_stats)) if len(fish_dn_stats) > 0 else np.nan
            ),
            "acf_lags": lags,
            "acf": acf,
            "acf_conf": conf_limit,
    ...
    def compute_residuals(self, dataset: PointProcessDataset) -> Dict[str, np.ndarray]:
        ...
        mu_pred = (
            self.compute_expected_rate(dataset)
            * dataset.n_fish_per_trial[:, None]
            * dataset.binning_dt
        )

        r = (
            self.dispersion_r
        )  # inf for Poisson/Hawkes/Renewal; finite for GammaPoissonProcess
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            ...
            deviance_res = np.sign(y_obs - mu_pred) * np.sqrt(
                np.maximum(0.0, deviance_sq)
            )
    ...
    def generate_model_predicted_counts(self, dataset, n_sims=1, rng=None):
        rng = rng or np.random.default_rng()
        fish_gains = self._draw_fish_gains(
            dataset.num_fish, n_sims, rng
        )  # shared per fish, per replicate

        predicted_counts = []
        for f_idx, t_idx, t_ev in dataset.iter_streams():
            for sim_i in range(n_sims):
                events = self.simulate_stream(
                    dataset, t_idx, fish_gains[f_idx, sim_i], rng
                )
    ...
    def plot_predicted_vs_observed(
        ...
        rng = rng or np.random.default_rng(
            0
        )  # fixed default seed: stable across repeated diagnose() calls

        observed = dataset.stream_event_counts
        simulated = self.generate_model_predicted_counts(
            dataset, n_sims=n_sims, rng=rng
        )

        ax = ax or plt.gca()
        max_k = (
            int(max(observed.max(), simulated.max()))
    ...
    def compute_diagnostics_data(
        ...
        """
        if self.params_ is None:
            raise ValueError("Model must be fitted before computing diagnostics.")
    ...
    def plot_panel_residual_surface(
        ...
        ax.set_title(
            "A. Deviance Residual Surface $(m, t)$", fontsize=11, fontweight="bold"
        )
    ...
    def plot_panel_calibration_ks(
        ...
        if n_rescaled == 0:
            ax.text(
                0.5,
                0.5,
                "No events available for calibration check",
                ha="center",
                va="center",
            )
            ax.set_title(
                "B. Time-Rescaling Calibration", fontsize=11, fontweight="bold"
            )
            return ax
        ...
        lower_S, upper_S = self.bootstrap_pooled_survival_band(
            tr_data["fish_pairs"], grid=grid_r_boot
        )
        ...
        ax.plot(
            [0, 1], [ks_bound, 1 + ks_bound], "k:", alpha=0.5, label="95% KS Limits"
        )
        ax.plot([0, 1], [-ks_bound, 1 - ks_bound], "k:", alpha=0.5)
        ax.set_xlim([0, 1])
        ax.set_ylim([0, 1])
        ax.set_title(
            "B. Time-Rescaling: Rescaled-Interval CDF vs. Uniform(0,1)",
            fontsize=11,
            fontweight="bold",
        )
    ...
    def plot_panel_residual_histogram(
        ...
        ax.set_title(
            "C. Deviance Residual Distribution", fontsize=11, fontweight="bold"
        )
    ...
    def plot_panel_event_lag_acf(
        ...
        ax.set_title(
            "D. Time-rescaled event autocorrelation (Event Lag)",
            fontsize=11,
            fontweight="bold",
        )
    ...
    def plot_parametric_gof_cox_snell(
        ...
        if not observed["has_censoring"]:
            raise ValueError(
                "plot_parametric_gof_cox_snell is intended for " "censored residuals."
            )
        ...
        def cdf_to_cox_snell(cdf: np.ndarray) -> np.ndarray:
            ...
            valid = np.isfinite(survival) & (survival > 0.0) & (survival <= 1.0)
        ...
        if show_pointwise_band:
            pointwise_lower = cdf_to_cox_snell(gof_result["bootstrap_cdf_lower"])
            pointwise_upper = cdf_to_cox_snell(gof_result["bootstrap_cdf_upper"])
        ...
        simultaneous_lower = cdf_to_cox_snell(gof_result["simultaneous_cdf_lower"])
        simultaneous_upper = cdf_to_cox_snell(gof_result["simultaneous_cdf_upper"])
        ...
        bootstrap_median = cdf_to_cox_snell(gof_result["bootstrap_cdf_median"])

        valid_median = np.isfinite(r_grid) & np.isfinite(bootstrap_median)
        ...
        if len(valid_observed) > 1:
            valid_observed[1:] &= observed_risk[1:] >= min_km_at_risk

        observed_neg_log_survival = -np.log(observed_survival[valid_observed])
        ...
        finite_bootstrap_r = r_grid[valid_simultaneous | valid_median]

        candidate_xmax = [observed_r_limit]

        if len(finite_bootstrap_r) > 0:
            candidate_xmax.append(float(np.max(finite_bootstrap_r)))
        ...
        if len(observed_neg_log_survival) > 0:
            finite_y.append(float(np.max(observed_neg_log_survival)))
        if np.any(valid_median):
            finite_y.append(float(np.max(bootstrap_median[valid_median])))
        if np.any(valid_simultaneous):
            finite_y.append(float(np.max(simultaneous_upper[valid_simultaneous])))

        y_max = max(finite_y + [x_max, 1e-6])
        ...
        summary = gof_result["summary"]
        ks_rows = summary.loc[summary["statistic"] == "calibration_ks"]
        ...
        if len(ks_rows) > 0:
            ...
            if np.isfinite(ks_value) and np.isfinite(p_value):
                statistic_text = f"KS={ks_value:.3f}, bootstrap p={p_value:.3f}"
    ...
    def plot_panel_calibration_cox_snell(
        ...
        ax.set_title(
            f"B. Cox-Snell Residual Plot (N={n_rescaled}, "
    ...
    def plot_panel_survival_curve(
        ...
        ax.set_title(
            "D. First-Event Survival Curve: Empirical vs Model",
            fontsize=11,
            fontweight="bold",
        )
        ax.legend(loc="upper right", fontsize=9)
    ...
    def plot_panel_residual_acf2d(
        ...
        extent_2d = [
            time_lags_sec[0] - dt / 2,
            time_lags_sec[-1] + dt / 2,
            trial_lags[0] - 0.5,
            trial_lags[-1] + 0.5,
        ]
        im_2d = ax.imshow(
        ...
        ax.set_title(
            "E. Autocorrelation of deviance residuals $R(\\Delta m, \\Delta t)$",
            fontsize=11,
            fontweight="bold",
        )
    ...
    def plot_panel_fish_dn_distribution(
        ...
        if len(dn_values) > 0:
            ...
            ax.axvspan(
                0.0, 0.05, color="green", alpha=0.1, label="Good Fit ($D_n < 0.05$)"
            )
            ax.set_title(
                f"F. Per-Fish $D_n$ Distribution ($N_{{fish}}={len(dn_values)}$)",
                fontsize=11,
                fontweight="bold",
            )
            ...
            ax.text(
                0.5,
                0.5,
                "Insufficient events per fish for $D_n$",
                ha="center",
                va="center",
            )
        return ax
...
class ModelComparator:
    ...
    def likelihood_ratio_test(
        model_null: PointProcess, model_alt: PointProcess
    ...
    def compare(
        ...
        **kwargs,
        ...
        if null_model is not None:
            null_fitted, null_error = _fit_one_model(
                null_model, dataset, method, kwargs
            )
        ...
        for model, error in results:
            if error is None:
                ...
                print(
                    f"[ModelComparator] WARNING: fit failed for {model.name}: {error}"
                )
                records.append(
                    {
                        "Model Name": model.name,
...
class ModelPlotter:
    ...
    def _weighted_marginals(
        surface: np.ndarray, dataset: PointProcessDataset
    ) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def plot_histogram(
        ...
        vmax = max(vmax, 1e-9)
        time_ymax = max(
            np.nanmax(x) for x in [tm_emp, tm_model] if np.any(np.isfinite(x))
        )
        trial_xmax = max(
            np.nanmax(x) for x in [trm_emp, trm_model] if np.any(np.isfinite(x))
        )
        ...
        ax_mod.set_title(
            f"Fitted Surface: {model.name}", fontsize=11, fontweight="bold"
        )
        ...
        ax_trial.plot(
            trm_emp, trial_centers, color="black", linestyle="-", linewidth=1.3
        )
        ax_trial.plot(
            trm_model, trial_centers, color="crimson", linestyle="--", linewidth=1.3
        )
    ...
    def plot_model_fits(
        ...
        for idx, model in enumerate(models):
            ...
            mean_pred = np.average(
                pred_surface, axis=0, weights=dataset.n_fish_per_trial
            )
